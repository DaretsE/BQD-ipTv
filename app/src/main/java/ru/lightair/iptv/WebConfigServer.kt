...
class WebConfigServer(private val onChanged: () -> Unit) : NanoHTTPD(PORT) {
    ...
    private fun handlePost(session: IHTTPSession): Response {
        ...
        when (session.uri) {
            "/api/add" -> {
                val url = p["url"]?.trim() ?: ""
                if (url.startsWith("http")) { Store.addPlaylist(p["name"]?.trim() ?: "", url); onChanged(); return ok() }
                return bad("Ссылка должна начинаться с http")
            }
            "/api/del" -> { Store.removePlaylist(p["url"] ?: ""); onChanged(); return ok() }
            "/api/hide" -> {
                val list = Store.getPlaylistCfgs()
                list.find { it.url == p["url"] }?.let { it.hidden = p["hidden"] == "1" }
                Store.savePlaylistCfgs(list); onChanged(); return ok()
            }
            "/api/rename" -> {
                val list = Store.getPlaylistCfgs()
                list.find { it.url == p["url"] }?.let { c -> p["name"]?.trim()?.takeIf { it.isNotEmpty() }?.let { c.name = it } }
                Store.savePlaylistCfgs(list); onChanged(); return ok()
            }
            "/api/epgadd" -> {
                val url = p["url"]?.trim() ?: ""
                if (url.startsWith("http")) { Store.addEpgSource(url); onChanged(); return ok() }
                return bad("Ссылка должна начинаться с http")
            }
            "/api/epgdel" -> { Store.removeEpgSource(p["url"] ?: ""); onChanged(); return ok() }
            "/api/favorder" -> {
                try {
                    ...
                    Store.saveFavorites(newList); Store.favManualOrder = true; onChanged(); return ok()
                } catch (e: Exception) { return bad(e.message ?: "err") }
            }
            "/api/favdel" -> { Store.saveFavorites(Store.getFavorites().filter { it.first != p["url"] }); onChanged(); return ok() }
            "/api/favauto" -> { Store.favManualOrder = false; onChanged(); return ok() }
        }
        return bad("unknown")
    }
    ...
    private fun jsonState(): Response {
        ...
        o.put("playlists", pls)
        val epg = JSONArray()
        for (s in Store.getEpgSources()) epg.put(s)
        o.put("epg", epg)
        val favs = JSONArray()
...
<title>Лёгкий Эфир</title>
<style>
:root{--bg:#0b0f14;--card:#161c24;--card2:#1d2530;--line:#2a3340;--text:#e9eef5;--muted:#8a97a8;--acc:#3b82f6;--gold:#f5b50a;--red:#ef4444;--green:#22c55e;}
*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:var(--bg);color:var(--text);margin:0;padding:0 0 40px;}
.top{position:sticky;top:0;z-index:10;background:linear-gradient(135deg,#0d2a52,#1a73e8);padding:20px 18px 16px;box-shadow:0 4px 20px rgba(0,0,0,.4)}
.top h1{margin:0;font-size:21px;font-weight:700;letter-spacing:.2px}
.top p{margin:4px 0 0;font-size:13px;opacity:.85}
.wrap{max-width:560px;margin:0 auto;padding:0 14px}
.sec{margin-top:22px}
.sec>h2{font-size:13px;text-transform:uppercase;letter-spacing:1px;color:var(--muted);margin:0 0 10px 2px;font-weight:600}
...
button{padding:12px 16px;border:none;border-radius:12px;background:var(--acc);color:#fff;font-size:15px;font-weight:600;width:100%;margin-top:4px;cursor:pointer;transition:.15s}
button:active{transform:scale(.98)}
button.sec-btn{background:var(--card2);border:1px solid var(--line)}
...
.iconbtn{width:38px;height:38px;border-radius:10px;padding:0;font-size:16px;background:var(--card2);border:1px solid var(--line);flex:none}
...
<div class="top"><h1>📺 Лёгкий Эфир</h1><p>Пульт управления контентом · изменения сразу на ТВ</p></div>
<div class="wrap">
  ...
  <div class="sec"><h2>Программа передач (EPG · XMLTV)</h2>
    <div class="card">
      ...
      <div class="hint">Можно добавить несколько источников — они объединятся. Если в плейлисте уже указан url-tvg, он подхватится сам.</div>
    </div>
    <div id="epgs"></div>
  </div>
...
function render(s){
 ...
 var e='';
 (s.epg||[]).forEach(function(u){
  e+='<div class="card"><div class="row"><div class="grow"><div class="url" style="font-size:13px;color:var(--text)">'+esc(u)+'</div></div>'+
     '<button class="iconbtn red" onclick="delEpg(\''+enc(u)+'\')">🗑</button></div></div>';
 });
...
function renamePl(u){var n=prompt('Новое название:');if(n)post('/api/rename',{url:decodeURIComponent(u),name:n}).then(load);}
...
function addEpg(){var u=document.getElementById('epgurl').value;if(!u){toast('Вставьте ссылку EPG');return;}
 post('/api/epgadd',{url:u}).then(function(r){if(r.ok){toast('✔ Источник добавлен');document.getElementById('epgurl').value='';}else toast(r.error);load();});}
